...
export const useFinancesStore = defineStore('finances', () => {
  ...
  function calculateWalletBalance(walletId) {
    ...
    // Get initial balance - prefer explicit initialBalance, fallback to balance only if initialBalance is truly not set
    // The initialBalance field stores the user's starting amount (Started With in Accounts page)
    let initialBalance = 0

    // Check if initialBalance is explicitly set and valid (not undefined, not null)
    // We need to handle the case where wallet.balance was already calculated from transactions
    const hasExplicitInitialBalance =
      wallet.initialBalance !== undefined &&
      wallet.initialBalance !== null &&
      !isNaN(Number(wallet.initialBalance))

    if (hasExplicitInitialBalance) {
      // initialBalance is explicitly set - use it
      initialBalance = Number(wallet.initialBalance)
      console.log('🔍 calculateWalletBalance: Using explicit initialBalance:', initialBalance)
    } else {
      // initialBalance not explicitly set - check if this is a new wallet (no transactions)
      const walletTransactions = transactions.value.filter((t) => t.walletId === walletId)

      if (walletTransactions.length === 0) {
        // No transactions yet - this is a new wallet, use balance as initial
        initialBalance = Number(wallet.balance) || 0
        console.log(
          '🔍 calculateWalletBalance: No transactions, using balance as initial:',
          initialBalance,
        )
      } else {
        // Has transactions but no explicit initialBalance
        // This is a legacy wallet - we need to infer the initialBalance
        // Calculate what the transactions net is
        ...
        const transactionNet = regularTransactions.reduce((total, transaction) => {
          ...
          else if (transaction.kind === 'expense') return total - transaction.amount
          return total
        }, 0)

        // The current balance (wallet.balance) should be: initialBalance + transactionNet
        // So: initialBalance = wallet.balance - transactionNet
        const currentBalance = Number(wallet.balance) || 0
        initialBalance = currentBalance - transactionNet

        // If initialBalance is negative, something is wrong - default to 0
        if (initialBalance < 0) {
          initialBalance = 0
          console.warn(
            '🔍 calculateWalletBalance: Calculated negative initialBalance, defaulting to 0',
          )
        } else {
          console.log(
            '🔍 calculateWalletBalance: Inferred initialBalance from wallet.balance:',
            initialBalance,
          )
        }
      }
